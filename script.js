(function() {
    ...
    const rBtn = document.getElementById('restart-btn'), transEl = document.getElementById('stage-transition');
    const transText = document.getElementById('transition-text'), transSub = document.getElementById('transition-subtitle');
    const skillText = document.getElementById('ui-skill-text');

    // Data dari Python (heroStats didefinisikan di app.py)
    const pData = typeof heroStats !== 'undefined' ? heroStats : {hp:3, spd:5, col:'#fff', type:'assault'};
    
    skillText.innerText = pData.type + " ULTIMATE";
    skillText.style.color = pData.col;
    uBar.style.background = pData.col;
    uBar.style.boxShadow = `0 0 10px ${pData.col}`;
    ...
    let pl = {
        x:300, y:200, r:12, speed:pData.spd, baseSpeed:pData.spd,
    ...
    function update(){
        ...
        // Skill Recharge Logic
        if(pl.type==='roket') pl.sT = bosses.length>0 ? Math.min(100,pl.sT+0.05) : (pl.kills/8)*100;
        else if(pl.type==='tank'||pl.type==='bomber') pl.sT=Math.min(100,pl.sT+0.1);
        else if(pl.type==='scout') pl.sT=Math.min(100,pl.sT+(bosses.length>0?0.2:0.1));
        else if(pl.type==='joker') pl.sT=(pl.kills/15)*100;
        else if(pl.type==='assault') pl.sT=Math.min(100,pl.sT+0.15);
            ...
            if(b.p){
                for(let i=bosses.length-1;i>=0;i--){
                    ...
                    if(Math.hypot(boss.x-b.x,boss.y-b.y)<boss.s+b.r){
                        if(!boss.shieldActive)boss.hp-=b.d; spawnExplosion(b.x,b.y,'#fff',5); return false;
                    }
                }
        ...
        bosses.forEach((boss, i) => {
            ...
            if(boss.hp<=0){
                ...
                spawnExplosion(boss.x,boss.y,boss.c,50);
                bosses.splice(i,1);
                if(boss.type==='main'){ showTrans('VICTORY!','Chapter Cleared!',180); setTimeout(advanceStage,3000); }
            }
        });
        ...
        // Spawning logic
        if(bosses.length===0 && enemies.length<8 && !gameOver){
            let ex, ey; do{ex=Math.random()*600; ey=Math.random()*400;} while(Math.hypot(ex-pl.x,ey-pl.y)<200);
            enemies.push({x:ex, y:ey, s:25, sp:1, hp:20, c:'#e74', val:10});
        }
        
        if(score>=1000 && score%1000<10 && bosses.length===0 && lastBossScore<score){
            lastBossScore=score+100; spawnMiniBoss();
        }
    }
    ...
    function draw(){
        ...
        particles.forEach(pt=>{ ctx.fillStyle=pt.c; ctx.globalAlpha=pt.life/30; ctx.fillRect(pt.x,pt.y,3,3); ctx.globalAlpha=1; });
        
        if(pl.inv<=0 || pl.inv%10<5){
            let ang=Math.atan2(my-pl.y,mx-pl.x);
            ctx.save(); ctx.translate(pl.x,pl.y); ctx.rotate(ang);
            ctx.fillStyle=pl.color; ctx.beginPath(); ctx.moveTo(18,0); ctx.lineTo(-12,-12); ctx.lineTo(-7,0); ctx.lineTo(-12,12); ctx.closePath(); ctx.fill();
            ctx.restore();
